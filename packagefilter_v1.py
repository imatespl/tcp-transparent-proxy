...
class PackageFilter(object):
    ...
    def get_filter_value(self):
        ...
        self.server_filter = str(self.cp.get('filter', 'server_filter'))
        self.client_filter = str(self.cp.get('filter', 'client_filter'))
...
class SockRedirectServer(StreamServer):
    ...
    def __init__(self, listener, package_f, **kwargs):
        ...
        self.filter_reply_db = shelve.open('/usr/local/share/filter_reply_db', 'c')
        self.reply_count = self.package_f.reply_package
        self.server_filter = self.package_f.server_filter
        self.client_filter = self.package_f.client_filter
    ...
    def handle(self, source, address):
        try:
            ...
            # change kernel use source ip query source neigh mac, not need get source mac from kernel. 
            #mac = source.getsockopt(socket.IPPROTO_IP, 172, 14)
            if self.filter_reply_db.has_key('count'):
                self.reply_count = self.filter_reply_db['count']
            # customer need new connection can connect to server  when client ip is blocked.
        ...
        #else:
        if self.client_filter == 'on' and self.server_filter == 'off':
            forwarders = (
                gevent.spawn(filter_forward, source, dest, self, self.package_f.package_filter, self.filter_db, self.filter_reply_db,
                            address[0], orgi_dst[0], False, self.reply_count),
                gevent.spawn(filter_forward, dest, source, self))
        elif self.server_filter == 'on' and self.client_filter == 'off':
            forwarders = (
                gevent.spawn(filter_forward, source, dest, self),
                gevent.spawn(filter_forward, dest, source, self, self.package_f.package_filter, self.filter_db, self.filter_reply_db,
                            orgi_dst[0], address[0], False, self.reply_count))
        elif self.server_filter == 'on' and self.client_filter == 'on':
            forwarders = (
                gevent.spawn(filter_forward, source, dest, self, self.package_f.package_filter, self.filter_db, self.filter_reply_db,
                            address[0], orgi_dst[0], False, self.reply_count),
                gevent.spawn(filter_forward, dest, source, self, self.package_f.package_filter, self.filter_db, self.filter_reply_db,
                            orgi_dst[0], address[0], False, self.reply_count))
        elif self.server_filter == 'off' and self.client_filter == 'off':
            forwarders = (
                gevent.spawn(filter_forward, source, dest, self),
                gevent.spawn(filter_forward, dest, source, self))
...
def filter_forward(source, dest=None, server=None, package_filter=None, filter_db=None, filter_reply_db=None,
                    src_ip=None, dst_ip=None, has_filtered=False, reply_count=None):
    drop_all = 0
    if has_filtered:
        drop_all = 1
                    ...
                    if package_filter is not None:
                        if reply_count:
                            ...
                            if response_data:
                                ...
                                filter_reply_db['count'] = reply_count
                                filter_reply_db.sync()
